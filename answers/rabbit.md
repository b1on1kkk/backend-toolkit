...
2. Обменник, получив сообщение, маршрутизирует его в одну или несколько очередей в соответствии с правилами привязки.

3. Очередь хранит ссылку на это сообщение. Само сообщение хранится в оперативной памяти или на диске.

4. Как только потребитель готов получить сообщение из очереди, сервер создает копию сообщения и отправляет.
...
Начиная с версии `RabbitMQ 2.4.0` алгоритм маршрутизации для **topic exchange** стал работать быстрее. Добились они этого путем внедрения подхода **trie implementation**.

Особенности:

- можно настроить таким образом, чтобы сообщения публиковались аналогично **direct exchange** или **fanout exchange**.
