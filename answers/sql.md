...
Блокировка — механизм (MVCC — Multi-Version Concurrency Control), который упорядочивает доступ к данным, когда два или более пользователей пытаются получить доступ к одним и тем же данным

- **Неявные** блокировки накладываются стандартными командами SQL и снимаются по умолчанию, когда транзакция фиксируется или откатывается.
- **Явные блокировки** накладываются по требованию пользователя конкретными запросами.

### Блокировки на уровне строк

| Блокировка            | Жёсткость        | Для чего                                           | Кто её блокирует                              |
| --------------------- | ---------------- | -------------------------------------------------- | --------------------------------------------- |
| **FOR UPDATE**        | 🔴 Самая жёсткая | Когда планируешь менять строку                     | UPDATE, DELETE, FOR UPDATE, FOR NO KEY UPDATE |
| **FOR NO KEY UPDATE** | 🟠 Мягче         | Когда меняешь неключевые поля                      | UPDATE ключей, DELETE, FOR UPDATE             |
| **FOR SHARE**         | 🟡 Ещё мягче     | Когда только читаешь, но нужна стабильность данных | UPDATE, DELETE, FOR UPDATE                    |
| **FOR KEY SHARE**     | 🟢 Самая мягкая  | Чтение с минимальными ограничениями                | DELETE, UPDATE ключей, FOR UPDATE             |


`SELECT … FOR UPDATE` — *“Я буду менять строку — никто её не трогает!”*

- Полностью блокирует изменения строки
- Нельзя даже сдвинуть уникальный ключ

`SELECT … FOR NO KEY UPDATE` — *“Я не трогаю ключ, просто обновлю что-то второстепенное”*

- Разрешает другим безопасные обновления ключей? → ❌ нет, ключи трогать нельзя
- Но **мягче**, чем `FOR UPDATE`

Обычно ставится автоматически, когда делаешь `UPDATE` без изменения ключевых полей.

`SELECT … FOR SHARE` — *“Я проверяю данные, пусть они будут неизменны”*

- Ты **не изменяешь** данные, но не даёшь другим изменить
- Но другие могут тоже **читать** и использовать `FOR SHARE`

`SELECT … FOR KEY SHARE` — *“Я просто читаю ключи — не ломайте ссылки”*

- Самая слабая блокировка
- Блокирует только:
  - `DELETE` (потому что строка исчезнет)
  - обновления ключевых полей (меняется идентичность строки)

### Блокировки на уровне таблиц

Есть еще блокировки на уровне таблиц, но я хз вообще когда они могут пригодится в работе, пока не сталкивался.

### Рекомендательные блокировки

`Advisory locks` — это механизм для создания пользовательских блокировок, которые приложение управляет самостоятельно, а не система автоматически. Они используются для синхронизации доступа к ресурсам на уровне бизнес-логики, не взаимодействуя с внутренними блокировками PostgreSQL (MVCC). Существуют блокировки на уровне транзакции (pg_advisory_xact_lock), которые снимаются при завершении транзакции, и блокировки на уровне сеанса (pg_advisory_lock), которые остаются активными до явного снятия или завершения сеанса. 
